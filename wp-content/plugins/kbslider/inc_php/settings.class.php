 ...
 */
	...
	class UniteSettings{
		...
		private function modifySettingsData($arrSettings){
			...
			foreach($arrSettings as $key=>$content){
				switch(getType($content)){
					...
					case "object":
					case "array":
						$content = UniteFunctions::convertStdClassToArray($content);
					break;					
				}
		...
		protected function getJsonClientString(){
		...
		public function addSap($text, $name="", $opened = false){
			...
			if(empty($text))
				UniteFunctions::throwError("sap $name must have a text");
		...
		public function addSection($label,$name=""){
			...
			if(!empty($this->arrSettings) && empty($this->arrSections))
				UniteFunctions::throwError("You should add first section before begin to add settings. (section: $text)");
				
			if(empty($label)) 
				UniteFunctions::throwError("You have some section without text");
		...
		protected function setSettingsStateByControls(){
		...
		private function getOptionsFromXMLField($field,$fieldName){
			...
			$options = UniteFunctions::getVal($arrField, "option");
			...
			foreach($options as $option){
				...
				if(gettype($option) == "string")
					UniteFunctions::throwError("Wrong options type: ".$option." in field: $fieldName");
				
				$attribs = $option->attributes();
				
				$optionValue = (string)UniteFunctions::getVal($attribs, "value");							
				$optionText = (string)UniteFunctions::getVal($attribs, "text");
				
				//validate options:
				UniteFunctions::validateNotEmpty($optionValue,"option value");
				UniteFunctions::validateNotEmpty($optionText,"option text");
		...
		public function loadXMLFile($filepath){
			...
			if(!file_exists($filepath))
				UniteFunctions::throwError("File: '$filepath' not exists!!!");
			
			$obj = @simplexml_load_file($filepath);
			
			if(empty($obj))
				UniteFunctions::throwError("Wrong xml file format: $filepath");
			...
			foreach($fieldsets as $fieldset){
				...
				$sapName = (string)UniteFunctions::getVal($attribs, "name");
				$sapLabel = (string)UniteFunctions::getVal($attribs, "label");
				
				UniteFunctions::validateNotEmpty($sapName,"sapName");
				UniteFunctions::validateNotEmpty($sapLabel,"sapLabel");
				...
				foreach($fields as $field){
					...
					$fieldType = (string)UniteFunctions::getVal($attribs, "type");
					$fieldName = (string)UniteFunctions::getVal($attribs, "name");
					$fieldLabel = (string)UniteFunctions::getVal($attribs, "label");
					$fieldDefaultValue = (string)UniteFunctions::getVal($attribs, "default");
					...
					$fieldDescription = (string)UniteFunctions::getVal($attribs, "description");
					$fieldClass = (string)UniteFunctions::getVal($attribs, "class");
					$fieldUnit = (string)UniteFunctions::getVal($attribs, "unit");
					$fieldRows = (string)UniteFunctions::getVal($attribs, "rows");
					$fieldCols = (string)UniteFunctions::getVal($attribs, "cols");
					$customType = (string)UniteFunctions::getVal($attribs, "custom_type");
					
					$options = $this->getOptionsFromXMLField($field,$fieldName);
					
					//validate must fields:
					UniteFunctions::validateNotEmpty($fieldType,"type");
					
					//validate name
					if($fieldType != self::TYPE_HR)
						UniteFunctions::validateNotEmpty($fieldName,"fieldName");
					...
					switch ($fieldType){
						...
						break;
						default:
							UniteFunctions::throwError("wrong type: $fieldType");
						break;						
					}
				...
				switch($type){
					case "custom.kenburns_position":
						...
						if(array_key_exists($name."_hor", $arrValues)){
							$value_vert = UniteFunctions::getVal($arrValues, $name."_vert","random");
							$value_hor = UniteFunctions::getVal($arrValues, $name."_hor","random");						
